// Nocturne komponente — Flutter prevod klasa iz `_ds/nocturne-.../styles.css`
// (.card, .card-kicker, .card-title, .tag, .table, .field, .dialog…).
//
// Pravilo: ove komponente NE znaju ništa o provider-ima niti o domenu.
// Primaju podatke kroz konstruktor i emituju callback-e. Sve boje, radijusi
// i spacing dolaze iz `config/app_theme.dart` — nigdje sirovi hex ili px.
...
/// Bool -> vizuelni status. Upute 7: status se prikazuje vizuelno,
/// nikad kao sirov `true`/`false` iz baze.
class NStatusTag extends StatelessWidget {
  ...
  const NStatusTag({
    ...
    this.activeLabel = 'Aktivan',
    this.inactiveLabel = 'Neaktivan',
  });
...
/// Pretraga sa ikonom. Upute 2.2: svaki pregled podataka mora imati
/// minimalno jedan parametar za pretragu.
...
/// Dropdown filter koji se puni iz proslijeđene liste.
/// Upute 6: padajuće liste se pune podacima iz baze, nikad hardkodirano —
/// zato ova komponenta ne sadrži nijednu vrijednost, sve prima kroz `items`.
  ...
  Widget build(BuildContext context) {
    ...
    if (rowCount == 0) {
      return Column(
        ...
        children: [
          header,
          empty ?? const NEmptyState(message: 'Nema zapisa za prikaz.'),
        ],
      );
    }
...
/// Ikonično dugme sa OBAVEZNIM tooltipom.
///
/// Upute 6: "Nedostupne akcije moraju imati onemogućeno (disabled) stanje uz
/// objašnjenje razloga nedostupnosti." Zato `disabledReason`: kad je
/// postavljen, dugme je disabled i tooltip objašnjava ZAŠTO.
...
/// Naslovna traka modala sa "X" u gornjem desnom uglu (Upute 6).
...
/// Confirmation dialog za nepovratne akcije (Upute 6).
...
/// Upute 4: nakon uspješne akcije prikazati SMISLENU poruku,
/// nikad generičko "Success" / "Bad request".
...
class NBarChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (data.isEmpty) {
      return SizedBox(
        height: height,
        child: const NEmptyState(message: 'Nema podataka za prikaz.'),
      );
    }
...
/// Horizontalni "meter" bar — koristi se u izvještajima
/// (Support Console dizajn: reservations-by-city).
