...
import 'package:ebooking_desktop/pages/manage_reservations.dart';
import 'package:ebooking_desktop/pages/manage_users.dart';
import 'package:ebooking_desktop/pages/messenger_screen.dart';
import 'package:ebooking_desktop/pages/my_profile_page.dart';
...
class _AppShellState extends State<AppShell> {
  ...
  IconData _iconFor(AdminSection section, {bool active = false}) {
    ...
    switch (section) {
      ...
      case AdminSection.reservations:
        return PhosphorIcons.calendarCheck(style);
      ...
      case AdminSection.myProfile:
        return PhosphorIcons.userCircle(style);
    }
  }
  ...
  Future<void> _logout() async {
    final confirmed = await nConfirm(
      context,
      title: 'Log out',
      message: 'Do you want to log out of the administrator account?',
    ...
    if (!mounted) return;
    nToast(context, result.message, isError: !result.serverNotified);
  }

  @override
  Widget build(BuildContext context) {
    // Otvaranje razgovora sa ekrana *Korisnici* postavlja `pendingChatId` u
    // `MessageProvider`; ovaj ekran ga prati i prebacuje na sekciju *Poruke*
    // kad se pojavi, umjesto da svaki pozivalac zna kako da promijeni sekciju.
    final hasPendingChat =
        context.watch<MessageProvider>().pendingChatId != null;
    if (hasPendingChat && _section != AdminSection.messenger) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) setState(() => _section = AdminSection.messenger);
      });
    }
  ...
  Widget _buildSection() {
    switch (_section) {
      ...
      case AdminSection.reservations:
        return const ManageReservationsPage();
      case AdminSection.users:
