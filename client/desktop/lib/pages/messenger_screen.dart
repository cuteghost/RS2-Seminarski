...
class _MessengerPageState extends State<MessengerPage> {
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) => _consumePendingChat());
  }

  Future<void> _consumePendingChat() async {
    if (!mounted) return;
    final pending = context.read<MessageProvider>().consumePendingChatId();
    if (pending != null) await _selectChat(pending);
  }
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      nToast(
        context,
        'The message was not sent. Check your connection to the messaging service '
        'and try again.',
        isError: true,
      );
    } finally {
    ...
    return Consumer2<MessageProvider, ProfileProvider>(
      builder: (context, messages, profileProvider, _) {
        ...
        if (messages.isLoading && messages.chats.isEmpty) {
          return const Center(child: NLoading(label: 'Connecting…'));
        }
  ...
  Widget build(BuildContext context) {
    return Center(
      child: NEmptyState(
        ...
        message: 'Select a conversation from the list to see the messages.',
      ),
    );
  }
}
      ...
      children: [
        ...
        Expanded(
          child: messages.isEmpty
              ? const NEmptyState(
                  message: 'The conversation is empty. Send the first message.',
                )
              : ListView.builder(
