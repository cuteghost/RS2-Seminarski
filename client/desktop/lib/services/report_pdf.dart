...
/// dostupna za preuzimanje i ispis." Prije redizajna desktop klijent NIJE
/// imao modul za izvještavanje — ovo je nova funkcionalnost.
///
...
class ReportPdf {
  ...
  /// Učitava i kešira temu — Upute 8.2: podatke koji se traže na svaki
  /// zahtjev treba keširati, a ne čitati iznova.
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          if (rows.isEmpty)
            _emptyNote('U bazi nema unesenih smještaja.')
          else
            _table(
              headers: const [
                '#',
              ...
              rows: [
                for (var i = 0; i < rows.length; i++)
                  [
                    '${i + 1}',
                    rows[i].name.isEmpty ? 'Bez naziva' : rows[i].name,
                    rows[i].typeOfAccommodation.label,
  ...
  static Future<Uint8List> reservationsByCity({
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        margin: const pw.EdgeInsets.fromLTRB(36, 40, 36, 44),
        header: (context) => _header(
          title: 'Rezervacije po gradu i državi',
        ...
        build: (context) => [
          _summary([
            ...
            ('Ukupan prihod', _moneyFmt.format(totalRevenue)),
            ('Broj gradova', '${data.length}'),
          ]),
          pw.SizedBox(height: 18),
          if (data.isEmpty)
            _emptyNote('Nema rezervacija u posmatranom periodu.')
          else ...[
            _table(
              headers: const [
                'Grad',
            ...
            pw.Text(
              'Prihod je izračunat kao cijena noćenja smještaja pomnožena '
              'brojem noćenja rezervacije. Rezervacije sa istim datumom '
              'dolaska i odlaska računaju se kao jedno noćenje.',
