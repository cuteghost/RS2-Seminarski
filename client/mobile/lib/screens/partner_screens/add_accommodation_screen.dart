...
// Amenity keys grouped the way guests actually filter on them, instead of
// fourteen ungrouped checkboxes in alphabetical/arbitrary order.
...
class AddAccommodationScreenState extends State<AddAccommodationScreen> {
  ...
  bool get _locationValid =>
      _selectedCountry != null && _selectedCity != null && _addressController.text.isNotEmpty;
  ...
  Future<void> _submit() async {
    if (images.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Add at least one photo before publishing.')),
      );
      return;
    }
    ...
    try {
      ...
      accommodationProvider.addAccommodation(accommodation);
      if (!mounted) return;
      ...
      await showDialog<void>(
        ...
        builder: (context) => AlertDialog(
          ...
          content: Text('${_nameController.text} is now live.'),
          actions: [
            TextButton(
              ...
              children: [
                if (_currentStep == 2)
                  Padding(
                    ...
                    child: Text(
                      '${_accommodationDetails.values.where((v) => v).length} selected',
    ...
    switch (_currentStep) {
      ...
      case 2:
        return _AmenitiesStep(
          details: _accommodationDetails,
          bedsController: _bedsController,
          onToggle: (key, value) => setState(() => _accommodationDetails[key] = value),
        );
      default:
        return _PhotosStep(
          images: images,
    ...
    return Column(
      ...
      children: [
        TextFormField(
          ...
          keyboardType: TextInputType.number,
          validator: (value) =>
              (value == null || value.isEmpty) ? 'Please enter the number of beds' : null,
        ),
        const SizedBox(height: 20),
        Text('Tap everything your place offers. Guests filter on these.',
            style: textTheme.bodySmall),
          ...
          Wrap(
            ...
            children: [
              for (final key in group.value)
                _AmenityToggle(
                  label: key,
                  selected: details[key] ?? false,
                  onTap: () => onToggle(key, !(details[key] ?? false)),
                ),
            ],
          ),
          const SizedBox(height: 20),
...
class _PhotosStep extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Add at least one photo. You can add more later.',
            style: textTheme.bodySmall),
        const SizedBox(height: 16),
        GridView.builder(
          shrinkWrap: true,
